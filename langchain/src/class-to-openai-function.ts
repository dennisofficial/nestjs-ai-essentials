...
export interface ClassOutputParserOptions {
  name?: string;
  description: string;
  v2?: boolean;
}
...
type ConstructorType<T> = T extends Constructor<infer U> ? U : never;
...
export interface ArraySchema<T> extends GenericSchema<T> {
  type: 'array';
...
const getProperties = (proto_keys: string[], target: any, v2: boolean = false) => {
  if (v2) {
    const schema = proto_keys.reduce((acc, key) => {
...
const handleType = (type: Constructor<unknown> | Enum<unknown>): Record<string, any> => {
  ...
  if (!['string', 'boolean', 'number', 'array'].includes(type_name)) {
    ...
    const required = Object.keys(properties).filter(
      (key) => !Reflect.getMetadata(KEY_PROP_OPTIONAL, type.prototype, key),
    );
...
export const AiArray =
  <C extends Constructor<object> | Enum<unknown>>(
    description: string,
    types: [C],
    options?: PropertyOptions<ArraySchema<ConstructorType<C>[]>>,
  ): PropertyDecorator =>
  (target: object, propertyKey: string | symbol) => {
    pushClassKey(target, propertyKey);

    if (Number(types.length) === 0) {
      throw new Error('The provided type array is empty. Provide one type for the array.');
    }

    const json_schema: ArraySchema<ConstructorType<C>[]> = {
      type: 'array',
...
export const AiToolCall =
  (options: ClassOutputParserOptions): ClassDecorator =>
  (target) => {
...
export const AiProp =
  (schema: JSONSchema): PropertyDecorator =>
  (target: object, propertyKey: string | symbol) => {
    pushClassKey(target, propertyKey);

    // Set OpenAI properties
    Reflect.defineMetadata(KEY_PROP_OPTIONS, schema, target, propertyKey);
  };

export const classToToolCall = (target: any): FunctionDefinition => {
  const proto_keys: string[] = Reflect.getMetadata(KEY_CLASS_KEYS, target.prototype) ?? [];

  const function_call: ClassOutputParserOptions = Reflect.getMetadata(KEY_FUNC_OPTIONS, target);

  const parameters = getProperties(proto_keys, target.prototype, function_call.v2);

  return {
    name: function_call.name ?? target.constructor.name,
...
export interface LLMBinder {
  tool_choice: string;
  tools: BindToolsInput[];
}

export class ClassToolOutputParser<
  T extends new (...args: any) => any,
> extends BaseCumulativeTransformOutputParser<InstanceType<T>> {
  lc_namespace = ['langchain', 'output_parsers', 'openai_functions'];

  private readonly func: FunctionDefinition;

  constructor(readonly clazz: T) {
    super();
    this.func = classToToolCall(clazz);
  }

  protected _diff(prev: any | undefined, next: any) {
    return next;
  }
  ...
  async parsePartialResult(
    ...
    const argsString = tool_call?.function?.arguments;
    ...
    try {
      // Use partial-json to parse incomplete JSON from streaming
      const parsed = parsePartialJson(argsString);
      return parsed as InstanceType<T>;
    } catch {
      // If parsing fails, return undefined to skip this chunk
      return undefined;
    }
  }

  async parse(_text: string): Promise<InstanceType<T>> {
    throw new Error(
      'ClassToolOutputParser.parse() is not supported. Use parseResult() or parsePartialResult() instead.',
    );
  }

  async parseResult(generations: ChatGeneration[] | Generation[]): Promise<InstanceType<T>> {
    const generation = generations[0];

    if (!('message' in generation)) {
      throw new Error(`No ToolCalls in message: ${JSON.stringify(generation)}`);
    }

    const message = generation.message;

    if (!('tool_calls' in message)) {
      throw new Error(`No ToolCalls in message: ${JSON.stringify(message)}`);
    }
    const tool_calls = message.tool_calls as any[];
    if (!Array.isArray(tool_calls) || tool_calls.length === 0) {
      throw new Error(`No ToolCalls in message: ${JSON.stringify(message)}`);
    }
    const tool_call = tool_calls[0];
    ...
    return tool_call.args as InstanceType<T>;
  }

  bindLLM = (): LLMBinder => ({
    tool_choice: this.func.name,
    tools: [{ type: 'function', function: this.func }],
  });
}
