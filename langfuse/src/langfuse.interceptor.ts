...
import { RunTracer } from './langfuse.run-tracer';
import {
...
class LangfuseInterceptor implements NestInterceptor {
  ...
  private createObserver(context: ExecutionContext, request: Request): LangfuseSpan {
    ...
    const requestTraceMetadata = {
      headers: request.headers,
      hostname: request.hostname,
      ip: request.ip,
      controllerClass: context.getClass().name,
      controllerHandler: context.getHandler().name,
    };
    const traceMetadata = buildPropagatedTraceMetadata(requestTraceMetadata);
    const observationMetadata = buildObservationTraceMetadata(requestTraceMetadata);
